...
function hasAlready(parent, replacedSelector, selector) {
  ...
  const hoveredSelector = selector
    ...
    .map(o => o.trim())
  const returned = hoveredSelector.some(o => deepFlatten(nodes).indexOf(o.replace(/:hover/gi, ':focus')) >= 0)

  if (returned) {
    return true
  }

  return parent.nodes.some(i => i.type === 'rule' && i.selectors.indexOf(replacedSelector) !== -1)
}
